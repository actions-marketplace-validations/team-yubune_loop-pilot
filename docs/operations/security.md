...
選定ロジックは `src/model-selector.ts` に集約。決定的 (deterministic) で I/O 副作用なし、`tests/model-selector.test.ts` で 8 ケース網羅。
...
```
[pre-fix] Model tier=escalated model=claude-opus-4-7 reasons=p0_finding,previous_check_failure
```
...
ticket の元案には「同一 finding が 2 iter 連続再指摘」も escalation 条件として含まれていたが、現状の `src/loop-detector.ts:isLoop` が「履歴中に同一 hash があれば即 `loop_detected` で停止」する設計のため、この条件は escalation するより前に workflow 停止に分岐する。今回スコープ外とし、isLoop の閾値緩和と組み合わせた別チケットで扱う。

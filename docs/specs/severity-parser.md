...
> **`AUTO_REVIEW_SEVERITY_THRESHOLD=P2` / `P3` を使う場合の注意 (TY-268 #23):**
> フォールバックは P2 / P3 を検出しない。Codex がバッジ無し finding を出した場合、severity 不明として `unparseable` カウンタに計上され、auto-fix 対象には残らない。閾値を P2 / P3 に下げて運用する場合、Codex 側で severity badge を必ず付ける review prompt にしておくこと。バッジ無し P2 / P3 を後から拾いたい時は `docs/operations/security.md` の review prompt 設定を見直すか、Codex review 結果を手動でトリアージする。

...
パーサーは **P0 / P1 / P2 / P3** を識別する（TY-256）。実際に修正対象とする範囲は `AUTO_REVIEW_SEVERITY_THRESHOLD`（default `P3`）で制御する。

- `AUTO_REVIEW_SEVERITY_THRESHOLD=P3`（既定）: P0 / P1 / P2 / P3 すべてを対象
- `AUTO_REVIEW_SEVERITY_THRESHOLD=P2`: P0 / P1 / P2 を修正対象、P3 を skip（旧挙動互換）
...
> **`AUTO_REVIEW_SEVERITY_THRESHOLD=P3`（既定） / `P2` を使う場合の注意 (TY-268 #23):**
